create database gds_mysql_assignment_db3;
 use gds_mysql_assignment_db3;--Q 101.--Table: UserActivity
 ...
 select * from UserActivity;--Write an SQL query to show the second most recent activity of each
 user.If the user only has one activity, return that one. A user cannot
 perform more than one activity at the same time.Return the result table
 in any order.
 ...
 where (num <> 1 and rnk = 2) or (num = 1 and rnk = 1);--Q102.--Table: UserActivity--Write an SQL query to show the second most recent activity of each
 user.If the user only has one activity, return that one. A user cannot
 perform more than one activity at the same time. Return the result
 table in any order.
 select distinct username, activity, startDate, endDate
 from
 (select u.*,
...
--Q103.--STUDENTS table
 ...
 select * from STUDENTS;--Query the Name of any student in STUDENTS who scored higher than 75
 ...
 96),(99989,'Joe',9,3573);
 select * from Employee;--Write a query that prints a list of employee names (i.e.: the name
...
--Q105--TRIANGLES table
 ...
 select * from TRIANGLES;--Write a query identifying the type of each record in the TRIANGLES
 ...
 ELSE 'Scalene'
 END
 FROM TRIANGLES;--Q106.--EMPLOYEES table
...
miscalculated average monthly salaries), and round it up to the next
 ...
 EMPLOYEES;--Q107.--Employee table
 select MAX(salary*months), COUNT(*) from Employee where (salary *
 months) >= (select MAX(salary * months) from Employee);--Q108.--OCCUPATIONS table
 ...
 select * from OCCUPATIONS;--Query the number of occurrences of each occupation in OCCUPATIONS.
 ...
 ORDER BY SELECTNUMBER ASC, THETEXT ASC;--Q109.--Pivot the Occupation column in OCCUPATIONS so that each Name is
...
select
 ...
 select * from BST;--Write a query to find the node type of Binary Tree ordered by the
 ...
 END
 FROM BST
ORDER by N;--Q111 .--Given the table schemas below, write a query to print the
 ...
 employees. Order your output by ascending company_code.--Company
 ...
 select * from Company;--Lead_Manager
 ...
 insert into Lead_Manager VALUES ('LM1','C1'),('LM2','C2');--Senior_Manager
 ...
 select * from Senior_Manager;--Manager Table:
...
senior_manager_code VARCHAR(50),
 ...
 c.company_code, c.founder ORDER BY c.company_code;--Q112.--Write a query to print all prime numbers less than or equal to 1000.
 ...
 order by L);
--Q113.--P(R) represents a pattern drawn by Julia in R rows. The following
 pattern represents P(5):--*--* *--* * *--* * * *--* * * * *--Write a query to print the pattern P(20).
 SELECT SYS_CONNECT_BY_PATH(NULL, '* ') FROM DUAL CONNECT BY ROWNUM <=
 20 ORDER BY 1 DESC;--Q114.--P(R) represents a pattern drawn by Julia in R rows. The following
 pattern represents P(5):--* * * * *--* * * *--* * *--* *--*--Write a query to print the pattern P(20).
 SELECT SYS_CONNECT_BY_PATH(NULL, '* ') FROM DUAL CONNECT BY ROWNUM <=
 20 ORDER BY 1 DESC;
 SET @no_of_lines = 5 + 1;
 ...
 WHERE @no_of_lines > 0;--Q115.--STUDENTS TABLE--Query the Name of any student in STUDENTS who scored higher than 75
 Marks. Order your output by the last three characters of each name. If
 two or more students both have names ending in the same last three
 characters (i.e.: Bobby, Robby, etc.), secondary sort them by ascending
 ID.
...
marks int
 ...
 SELECT name FROM Employee ORDER BY name;--Q117..--Write a query that prints a list of employee names (i.e.: the name
 attribute) for employees in Employee having a salary greater than $2000
 per month who have been employees for less than 10 months. Sort your
 result by ascending employee_id.
 SELECT name FROM Employee WHERE salary > 2000 AND months < 10 ORDER BY
 employee_id;--Q118.----TRIANGLES table
 create table if not exists TRIANGLES
(
 A int,
 B int,
 ...
 ELSE 'Scalene'
 END
 FROM TRIANGLES;--Q119.--user_transactions Table:
 ...
 select * from user_transactions;--Write a query to obtain the year-on-year growth rate for the total
 spend of each product for each year.
--Q120.--inventory table:
 ...
 select * from inventory;--Write a SQL query to find the number of prime and non-prime items
 ...
 COUNT(*) AS item_count
 FROM inventory
 GROUP BY item_type;--Q121.--user_actions Table:
...
--Write a query to obtain the active user retention in July 2022.
 ...
 GROUP BY EXTRACT(MONTH FROM curr_month.event_date);--Q122.--search_frequency Table:
 ...
 select * from search_frequency;--Write a query to report the median of searches made by a user. Round
...
--Q123.--advertiser Table:
 ...
 select * from advertiser;--daily_pay Table:
 ...
 select * from daily_pay;--Write a query to update the Facebook advertiser's status using the
...
ON advertiser.user_id = payment.user_id
 ...
 END AS new_status
 FROM payment_status
 ORDER BY user_id;--Q124.--server_utilization Table:
 ...
 select * from server_utilization;--Write a query that calculates the total time that the fleet of
 servers was running. The output should be in units of full days.--Q125.--transactions
...
12:28:00'),(4,102,2,300,'09/25/2022 12:00:00'),(6,102,2,400,'09/25/2022
 14:00:00');
 select * from transactions;--Sometimes, payment transactions are repeated by accident; it could be
 due to user error, API failure or a retry error that causes a credit
 card to be charged twice. Using the transactions table, identify any
 payments made at the same merchant with the same credit card for the
 same amount within 10 minutes of each other. Count such repeated
 payments.
 WITH payments AS (
 SELECT
 merchant_id,
...
LAG(transaction_timestamp) OVER(
 ...
 FROM payments
 WHERE minute_difference <= 10;--Q126.--orders Table:
 ...
 13:43:00');
 select * from orders;--trips Table:
...
(
 ...
 14:10:00');
 select * from trips;--customers Table:
 ...
 select * from customers;--Write a query to find the bad experience rate in the first 14 days
 for new users who signed up in June 2022. Output the percentage of bad
 experience rounded to 2 decimal places.--Q127.-- Scores
 ...
 M','2020-01-07',7),('Khali','M','2019-12-25',11),('Slaman','M','2019-12-30',13),('Joe','M','2019-12-31',3),('Jose','M','2019-12-18',2),('Priya
...
--Write an SQL query to find the total score for each gender on each
 ...
 order by s1.gender, s1.day;--Q128.--Table Person:
 ...
 hel','972-0011100');
 select * from Person;--Country table:
 ...
 51);
 select * from Country;--Table Calls:
...
insert into Calls VALUES
 ...
 3),(9,7,1),(1,7,7);
 select * from Calls;--Write an SQL query to find the countries where this company can
 ...
 co.name
 HAVING
 AVG(duration) > (SELECT AVG(duration) FROM Calls);--Q129.--Table: Numbers
 ...
 select * from Numbers;--Write an SQL query to report the median of all the numbers in the
 ...
 tot_count/2 and tot_count/2+1;--Q130.--Table: Salary
 ...
 select * from Employee;--Write an SQL query to report the comparison result
...
case when dept_avg > comp_avg then 'higher' when dept_avg <
 ...
 select * from Activity;--Write an SQL query to report for each install date, the number of
 players that installed the game on that day, and the day one
 retention.Return the result table in any order.
...
--Q132.--Table: Players
 ...
 (35,2), (50,2), (20,3), (40,3);
 select * from Players;--Table: Matches
 ...
 (1,15,45,3,0),(2,30,25,1,2),(3,30,15,2,0),(4,40,20,5,2),(5,35,50,1,1);--Write an SQL query to find the winner in each group.--Return the result table in any order.
 ...
 order by group_id, score desc, player_id-- limit 1-- by default, groupby will pick the first one i.e.
...
--Q133.--Table: Student
 ...
 select * from Student;--Table: Exam
 ...
 select * from Exam;--Write an SQL query to report the students (student_id, student_name)
 ...
 and min(score) not in (select min(score) from Exam);--Q134.--Table: Student--Exam table:--Write an SQL query to report the students (student_id, student_name)
 being quiet in all exams. Do not return the student who has never taken
 any exam.Return the result table ordered by student_id.
 select
 Student.*
from Exam
 ...
 group by student_id
 having max(score) not in (select max(score) from Exam)
 and min(score) not in (select min(score) from Exam);--Q135.--Table: UserActivity
 create table if not exists UserActivity
 (
 username VARCHAR(50),
 activity varchar(50),
 ...
 count(activity) over (partition by username) as num
 from UserActivity u) t
 where (num <> 1 and rnk = 2) or (num = 1 and rnk = 1);--Q136.--Table: UserActivity--Write an SQL query to show the second most recent activity of each
 user.If the user only has one activity, return that one. A user cannot
 perform more than one activity at the same time. Return the result
 table in any order.
 select distinct username, activity, startDate, endDate
 ...
 count(activity) over (partition by username) as num
from UserActivity u) t
 where (num <> 1 and rnk = 2) or (num = 1 and rnk = 1);--Q137.--EMPLOYEES table
 create table if not exists EMPLOYEES
 (
 id int,
 name VARCHAR(50),
 ...
 insert into EMPLOYEES VALUES (1,'Kristeen',1420), (2,'Ashley',2006),
 (3,'Julia',2210), (4,'Maria',3000);
 select * from EMPLOYEES;--Write a query calculating the amount of error (i.e.: actual
miscalculated average monthly salaries), and round it up to the next
 integer.
 select ceil(avg(salary)- avg(replace(salary, '0', ''))) from
 EMPLOYEES;--Q138.--Employee table
 create table if not exists Employee
 (
 employee_id int,
 name VARCHAR(50),
 ...
 72),(78454,'Bonnie',8,1771),(83565,'Michael',6,2017),(98607,'Todd',5,33
 96),(99989,'Joe',9,3573);
 select * from Employee;--Write a query to find the maximum total earnings for all employees as
 well as the total number ofemployees who have maximum total earnings.
 Then print these values as 2 space-separated integers.
 select MAX(salary*months), COUNT(*) from Employee where (salary *
 months) >= (select MAX(salary * months) from Employee);
--Q139.--OCCUPATIONS table
 create table if not exists OCCUPATIONS
 (
 Name VARCHAR(50),
 Occupation VARCHAR(50)
 ...
 ORDER BY SELECTNUMBER ASC, THETEXT ASC;--Q140 .--Pivot the Occupation column in OCCUPATIONS so that each Name is
 sorted alphabetically and displayed underneath its corresponding
 Occupation. The output column headers should be Doctor, Professor,
 Singer, and Actor, respectively.
 select
...
select
 ...
 END
 FROM BST
 ORDER by N;--Q142 .--Given the table schemas below, write a query to print the
 company_code, founder name, total number of lead managers, total number
 of senior managers, total number of managers, and total number of
 employees. Order your output by ascending company_code.--Company
 create table if not exists Company
(
 ...
 select * from Lead_Manager;--Senior_Manager
 create table if not exists Senior_Manager
 (
 senior_manager_code VARCHAR(50),
 lead_manager_code VARCHAR(50),
...
(
 ...
 COUNT(DISTINCT e.employee_code) FROM Company c
 JOIN Employee e ON c.company_code = e.company_code GROUP BY
 c.company_code, c.founder ORDER BY c.company_code;--Q143.--Functions table
 ...
 select * from Functions;--Write a query to output all such symmetric pairs in ascending order
 by the value of X. List the row such that X1 ≤ Y1.
...
--Students TABLE
 ...
 select * from Students;--Friends TABLE
 ...
 select * from Friends;--Packages. TABLE
 ...
 insert into Packages VALUES (1,15.20),(2,10.06),(3,11.55),(4,12.12);--Write a query to output the names of those students whose best
...
--Hackers table:
 ...
 odd'),(90411,'Joe');
 select * from Hackers;--Difficulty: table:
 ...
 select * from Difficulty;--Challenges table:
 ...
 243,2);
 select * from Challenges;--Submissions table:
...
submission_id int,
 ...
 select * from Submissions;--Write a query to print the respective hacker_id and name of hackers
 who achieved full scores for more than one challenge. Order your output
 in descending order by the total number of challenges in which the
 hacker earned a full score. If more than one hacker received full
 ...
 ORDER BY count(S.challenge_id) DESC, S.hacker_id;--Q146.--table Projects
 ...
 (1,'2015-10-01','2015-10-02'),(2,'2015-10-02','2015-10-03'),(3,'2015-10-03','2015-10-04'),(4,'2015-10-13','2015-10-14'),(5,'2015-10-14','2015
10-15'),(6,'2015-10-28','2015-10-29'),(7,'2015-10-30','2015-10-31');--Write a query to output the start and end dates of projects listed by
...
If there is more than one project that have the same number of
 ...
 GROUP BY Start_Date
 ORDER BY datediff(MIN(End_Date), Start_Date), Start_Date;--Q147.--transactions Table:
 create table if not exists transactions
 (
 user_id int,
 amount float,
 ...
 select * from transactions;--In an effort to identify high-value customers, Amazon asked for your
 help to obtain data about users who go on shopping sprees. A shopping
 spree occurs when a user makes purchases on 3 or more consecutive days.
 List the user IDs who have gone on at least 1 shopping spree in
 ascending order.--Q148 .--payments Table:--You are given a table of PayPal payments showing the payer, the
 recipient, and the amount paid. A two-way unique relationship is
 established when two people send money back and forth. Write a query to
 find the number of two-way unique relationships in this
 data.Assumption:
--A payer can send money to the same recipient multiple times.
 ...
 FROM
 T1;--Q149.--user_transactions Table:
 create table if not exists user_transactions
 (
 transaction_id int,
 product_id int,
...
--Write a query to obtain the list of customers whose first transaction
 was valued at $50 or more. Output the number of users.--Q150.--measurements Table:
 ...
 select * from measurements;--Write a query to obtain the sum of the odd-numbered and even-numbered
 ...
 GROUP BY measurement_day;--Q151.--transactions Table:--transactions Table:
create table if not exists transactions
 (
 user_id int,
 amount float,
 ...
 spree occurs when a user makes purchases on 3 or more consecutive days.
 List the user IDs who have gone on at least 1 shopping spree in
 ascending order.--Q152.--rental_amenities Table:
 ...
 (456,'pool');
 select * from rental_amenities;--write a query to find the unique combination of two Airbnb rentals
 with the same exact amenities offered.--Q153.--ad_campaigns Table:
...
select * from ad_campaigns;--Write a query to calculate the return on ad spend (ROAS) for each
 advertiser across all ad campaigns. Round your answer to 2 decimal
 places, and order your output by the advertiser_id.
 select advertiser_id, round(cast(sum(revenue)/sum(spend) as numeric),2)
 as ROAS
 from ad_campaigns
 group by advertiser_id
 order by advertiser_id;--Q154.--employee_pay Table:
 ...
 select * from employee_pay;--Write a query that shows the following data for each compensation
 outlier: employee ID, salary, and whether they are potentially overpaid
 or potentially underpaid (refer to Example Output below).--Q155.--payments table
 create table if not exists payments
 (
 payer_id int,
 recipient_id int,
...
WITH T1 AS
 ...
 00:00:00');
 select * from purchases;--Write a query to obtain the number of users who purchased the same
 product on two or more different days. Output the number of unique
 users.--Q157.--transactions Table:
 create table if not exists transactions
 (
...
insert into transactions VALUES (19153,'deposit',65.90,'07/10/2022
 ...
 select * from transactions;--Write a query to print the cumulative balance of the merchant account
 at the end of each day, with the total balance reset back to zero at
 the end of the month. Output the transaction date and cumulative
 balance.--Q158.--product_spend Table:
 ...
 select * from product_spend;--Identify the top two highest-grossing products within each category
...
SELECT
 ...
 WHERE ranking <= 2
 ORDER BY category, ranking;--Q159.--users Table:
 ...
 select * from users;--Write a query to generate the churn rate by week in June 2022. Output
 the week number (1, 2, 3, 4, ...) and the corresponding churn rate
